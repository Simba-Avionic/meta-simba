...
do_install:append() {
    ...
    install -d ${D}/bin
    install -d ${D}/opt/cpu_srp
    
    install -m 0777 ${WORKDIR}/srp_start.sh  ${D}/bin

    install -m 0777 ${WORKDIR}/network_interface.sh ${D}/opt/cpu_srp
    install -m 0777 ${WORKDIR}/start_up.sh  ${D}/opt/cpu_srp
...
FILES:${PN} += "\
    ...
    /opt/cpu_srp/start_up.sh \
    /opt/cpu_srp/network_interface.sh  \
"
